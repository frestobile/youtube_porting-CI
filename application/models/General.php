...
class General extends CI_Model {
    ...
    function total_rows($tb_name, $cond) {
        $query = $this->db->get_where($tb_name, $cond);
        return $query->num_rows();
    }
   
    function get_recommand_items($tb_name,$cond, $order, $limit) {
    ...
    function get_video_items($tb_name,$cond, $order) {
        $this->db->order_by($order, 'desc');
        $query = $this->db->get_where($tb_name,$cond);
        return $query->result();
    }
    ...
    function get_item_search($tb_name,$cond, $order,$keyword,$date_from,$date_end) {
        $this->db->order_by($order, 'desc');
    ...
    function get_category_search($tb_name,$cond, $order,$category,$date_from,$date_end) {
        $this->db->order_by($order, 'desc');
    ...
    function get_cate_recommand_items($tb_name,$cond, $order) {
        $this->db->order_by($order, 'desc');
        $query = $this->db->get_where($tb_name, $cond);
        return $query->result();
    }
    function get_recommand_item_images($recommand_items){
        
        $items_images_arr = array();
        foreach ($recommand_items as $item){           
            $cond['item_id'] = $item->id;
            $item_image = $this->General->get_row('item_images', $cond);            
            array_push($items_images_arr, $item_image->img_url);
        }
        
        return $items_images_arr;
    }
    //get all rows
    ...
    function add_user($tb_name, $data) {
        $result = $this->db->insert($tb_name, $data);
        return $result;
    }

    function check_user($data) {
        $result = $this->db->get_row('users', $data);
        return $result;
    }
